    ...
    for signal in antenna_dict.keys():
        ...
        for i in range(len(antenna_dict[signal])):
            for j in range(i + 1, len(antenna_dict[signal])):
                ...
                while is_in_range(anti_node_one, y_max, x_max):
                    result.add(anti_node_one)
                    anti_node_one = (
                        anti_node_one[0] + dir_vector[0],
                        anti_node_one[1] + dir_vector[1],
                    )

                while is_in_range(anti_node_two, y_max, x_max):
                    result.add(anti_node_two)
                    anti_node_two = (
                        anti_node_two[0] - dir_vector[0],
                        anti_node_two[1] - dir_vector[1],
                    )

                # since they're a pair, it satisfies at least two antennas in line
                result.add(antenna_one)
                result.add(antenna_two)
