...
def simulate_results(registers, program):
...
def solution():
    ...
    for i in range(len(program) - 1, -1, -1):
        for a in range(lower_bound, lower_bound + (8 ** (len(program) - i))):
            registers_copy = registers.copy()
            registers_copy[0] = a
            outputs = simulate_results(registers_copy, program)

            if program[i:] == outputs:
                # check if partial match or whole match
                if len(outputs) == len(program):
                    return a

                oct_a = dec_to_oct(a)
                # octal shifting to the left
                lower_bound = oct_to_dec(oct_a + "0")
                break
