...
def simulate_path(y_max, x_max, obstacles, src, dst):
    path_q = []
...
def solution():
    ...
    while best_path == None:
        # print(t)
        new_obstacle = byte_positions[t]
        obstacles.remove(new_obstacle)
        
        best_path = simulate_path(y_max, x_max, obstacles, src, dst)
        t -= 1
    
    # visualize(y_max, x_max, obstacles, best_path)

    return str(byte_positions[t + 1][1]) + ',' + str(byte_positions[t + 1][0])
